...
export function generateInvitationLink(classData) {
  ...
  const classCode = classData.classCode || classData.inviteCode || classData.code;
  if (!classCode) {
    console.error('No class code available for invitation link generation', classData);
    return '';
  }
  
  // Get the base URL - use environment variable if available for production
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || window.location.origin;
  const invitationUrl = `${baseUrl}/student/join/${classCode}`;
  return invitationUrl;
}

/**
 * Generates an NFC check-in link for a class
 * @param {Object} classData - The class information
 * @returns {string} The complete NFC check-in URL
 */
export function generateNFCCheckInLink(classData) {
  if (typeof window === 'undefined') {
    return ''; // Return empty string on server side
  }
  
  // Debug: Log the available properties
  console.log('Class data for NFC link:', {
    id: classData.id,
  ...
  // Use the classCode for NFC check-in (same as invitation link)
  const checkInCode = classData.classCode || classData.inviteCode || classData.code;
  if (!checkInCode) {
    console.error('No class code available for NFC link generation', classData);
    return '';
  }
  ...
  console.log('Generated NFC URL:', checkInUrl);
  return checkInUrl;
}
...
/**
 * Copies NFC check-in link to clipboard
 * @param {Object} classData - The class information
 * @returns {Promise<boolean>} Success status of copy operation
 */
export async function copyNFCCheckInLink(classData) {
  try {
    const nfcLink = generateNFCCheckInLink(classData);
    await navigator.clipboard.writeText(nfcLink);
    return true;
  } catch (error) {
    console.error('Failed to copy NFC check-in link:', error);
    return false;
  }
}
